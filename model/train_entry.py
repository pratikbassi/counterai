"""
Slim training entrypoint.

Most logic lives in helper modules:
- `train_data.py` (dataset discovery + Real/Fake loading)
- `train_transforms.py` (data augmentation + normalization)
- `train_loop.py` (train/eval loops, AMP, throughput logging)
- `train_model.py` (model creation)
"""
...
from train_data import build_train_val_datasets
from train_loop import evaluate, evaluate_confusion_matrix, train_one_epoch
...
def main() -> None:
    ...
    amp_enabled = device.type == "cuda" and (args.amp if args.amp is not None else True)
    ...
    # This prints per-class accuracy and a confusion matrix to help diagnose errors.
    print(
